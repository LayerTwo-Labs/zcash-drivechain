...
bool AppInit(int argc, char* argv[])
{
    ...
    {
        std::string strUsage = _("Zside Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n" + PrivacyInfo();
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  zsided [options]                     " + _("Start Zside Daemon") + "\n";
    ...
    try
    {
        ...
        try
        {
            ...
            fprintf(stderr,
                (_("Before starting zsided, you need to create a configuration file:\n"
                   ...
                   "depending on how you installed Zside:\n") +
                 _("- Source code:  %s%s\n"
                   "- .deb package: %s%s\n")).c_str(),
        ...
        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in zsided. Use the zside-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
        ...
        if (fDaemon)
        {
            fprintf(stdout, "Zside server starting\n");
